...
for (let i = 1; i <= 4; i++) {
  ...
  card.className =
    "bg-white shadow-md rounded-lg p-4 hover:shadow-lg transition";
...
button.className =
  "mt-6 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 focus:ring-2 focus:ring-blue-400";
...
for (let i = 1; i <= 3; i++) {
  ...
  td1.className = "border px-4 py-2";
  const td2 = document.createElement("td");
  td2.textContent = `User${i}`;
  td2.className = "border px-4 py-2";
  const td3 = document.createElement("td");
  td3.textContent = `user${i}@example.com`;
  td3.className = "border px-4 py-2";
  tr.appendChild(td1);
  tr.appendChild(td2);
  tr.appendChild(td3);
...
pArticle.textContent =
  "This article is nested inside a section with Tailwind styling.";
