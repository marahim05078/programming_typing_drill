...
items.forEach((item,index)=>{ item.style.backgroundColor=index%2===0?"#eee":"#ccc"; });
...
if(cardBody.parentNode){ cardBody.parentNode.replaceChild(replacement,cardBody); }
...
Array.from(container.children).forEach(child=>console.log("Child:",child.tagName));
...
inputUser.type="text"; inputUser.name="username"; inputUser.placeholder="Username";
const inputPass=document.createElement("input");
inputPass.type="password"; inputPass.name="password"; inputPass.placeholder="Password";
...
for(let i=1;i<=3;i++){
  ...
  const td2=document.createElement("td"); td2.textContent=`User${i}`;
  const td3=document.createElement("td"); td3.textContent=`user${i}@example.com`;
